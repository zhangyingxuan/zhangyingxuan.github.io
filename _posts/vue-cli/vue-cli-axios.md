...
```
* 你可以给cancelToken构造函数传递一个`executor function`来创建一个`cancel token:`
```
var cancelToken = axios.CancelToken;
var cance;
axios.get('/user/12345',{
...
* config/index.js 添加 proxyTable 属性
...
module.exports = {
  dev: {
    ...
    proxyTable: {
        '/api': {
          target: 'http://xxx', // 要调用的接口
          changeOrigin: true, // 是否跨域
          pathRewrite: {
             '^/api': ''  // 用‘/api’代替target里面的地址，后面组件中我们掉接口时直接用api代替
                          // 比如我要调用'http://xxx/app/account/login.json'，直接写‘/api/app/account/login.json’即可
          }
    }
```

2、配置开发和生产环境对应的接口地址
* 在config/dev.env.js和config/prod.env.js配置对应的API_HOST
```
// config/dev.env.js
...
2、 在 main.js 中导入 requestObject
```
import requestObject from 'network.js path'

Vue.prototype.$axios = requestObject
```

3、使用实例

```
// url: 与 APP 中相同传相对地址即可
// param: 参数传对象、json串皆可
// get、post 均是基于 Promise 实现，所以可通过 .then()、.catch() 链式调用的方式添加请求成功或失败后的处理方法

this.$axios.post('app/account/login.json', param)
  .then(function (response) {
...
// 可添加多个 .then() 方法，多个 .then() 会按添加顺序顺序执行
...
this.$axios.post('app/account/login.json', param)
  ...
  .then(function (res) {
    console.log("another success handle");
    return res;
  })
  .catch(function (error) {
    console.log("failed handle");
...
// 请求成功，控制台输出信息
// success handle
// another success handle

// .then() 中 return response 等价于 return Promise.resolve(reponse)，response 会作为参数传递给下一个 .then()
// .then() 中没有 return 值后续 .then() 依然会调用只是接受的参数为 undefined
// 在 .then() 中可通过 return Promise.reject() 来中断 Promise chain 跳过后续的 .then() 操作
// 在 Promise.reject() 中传入一个参数，用于在 .catch() 中判断是错误信息还是我们主动中断 Promise chain
