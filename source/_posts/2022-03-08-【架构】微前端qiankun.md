...
# 移动端可安装Vconsole（非必须）
# 【高阶用法】可通过reqHeaders或resAppend配置，传入header或
w2 i whistle.inspect
# 在whistle配置页rules使用
www.xxx.com http://127.0.0.1:3002 whistle.inspect://vConsole cache://no
```
  ...
  registerMicroApps(apps, {
    ...
    beforeMount: [
      (app) => {
        ...
        store.commit(
          `global/${storeGlobalTypes.SET_SIDEBAR_STATUS}`,
          app.props.isParentSlider
        ); // 设置顶栏
      },
    ],
    afterUnmount: [
...
registerMicroApps(apps, {
    beforeLoad: [
      (app) => {
        // 使用 Promise.all 确保所有依赖加载完成后再进入子应用生命周期
        await Promise.all(commonJsLibs.map(lib => loadScript(`${VUE_APP_ROUTER_BASE}/lib/${lib}`)));
        await Promise.all(commonCssLibs.map(lib => loadLink(`${VUE_APP_ROUTER_BASE}/lib/${lib}`)));
