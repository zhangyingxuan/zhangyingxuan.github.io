---
title: 2025-12-17-【工程化】SSR转SSG部署
categories:
  - 工程化
tags:
  - SSR
...
3. **部署到 1 核 1G 服务器：**
   将 `.output/public` 下的所有内容拷贝到 1 核 1G 服务器的 Nginx 目录下即可。
...
1. **1 核 1G 服务器**：压力几乎消失，Nginx 只负责静态分发。
2. **2 核 2G 服务器**：完全不需要运行 Nuxt 进程，MySQL 运行更稳。
3. **SEO**：每一个组合页面都有独立的静态 HTML，SEO 满分。
4. **体验**：用户在不同转换页面切换时，依然是 SPA 的流畅感。

**需要我帮您写一个自动将生成的静态文件夹上传至 1 核 1G 服务器的部署脚本（Shell）吗？**
